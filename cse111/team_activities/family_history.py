...
def print_death_age(people_dict):
    ...
    for value in people_dict.values():
        ...
        print(f"{name} was {age} when they died")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for value in marriages_dict.values():
        ...
        husband_age = value[WEDDING_YEAR_INDEX] - husband_info[BIRTH_YEAR_INDEX]

        wife_key = value[WIFE_KEY_INDEX]
        wife_info = people_dict[wife_key]
        wife_name = wife_info[NAME_INDEX]
        wife_age = value[WEDDING_YEAR_INDEX] - wife_info[BIRTH_YEAR_INDEX]

        wedding_year = value[WEDDING_YEAR_INDEX]

        print(f"{husband_name} ({husband_age}) married {wife_name} ({wife_age}) in {wedding_year}")
