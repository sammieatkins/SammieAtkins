...
def add_book(rows):
    ...
    while confirm == "n":
        ...
        if confirm == "y":
            ...
            ########## books_list.append() somehow? - for updating books list during program
        elif confirm == "n":
...
def sort_books(books_list):
    ...
    while display_choice != 4:
        ...
        # Sort by title.
        if display_choice == 1:
            sorted_list = sorted(books_list, key=lambda books_list:books_list[TITLE_INDEX][4:].upper() if books_list[TITLE_INDEX][:3].lower() == "the" else books_list[TITLE_INDEX].upper())
